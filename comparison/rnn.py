"""RNN 目标检测器 — YOLOv13 对比基线模型
使用循环神经网络进行序列化车辆检测，作为时序模型基线与 YOLOv13 对比

用法:
    python comparison/rnn.py
"""
...
class RNNDetector(nn.Module):
    def __init__(self):
        super().__init__()
        self.lstm = nn.LSTM(input_size=640 * 3, hidden_size=256, num_layers=2, batch_first=True, bidirectional=True)
        self.fc = nn.Sequential(
            nn.Linear(256 * 2, 512), nn.ReLU(),
            nn.Linear(512, 6)
        )

    def forward(self, x):
        x = x.permute(0, 2, 1, 3).reshape(x.shape[0], 640, -1)
...
def train_rnn():
    ...
    train_loader = DataLoader(train_dataset, batch_size=4, shuffle=True)  # 减小批次

    model = RNNDetector().to(device)
    ...
    for epoch in range(epochs):
        ...
        print(f"RNN Epoch [{epoch + 1}/{epochs}], Loss: {avg_loss:.4f}, Time: {epoch_time:.2f}s")

    # 保存模型和日志
    torch.save(model.state_dict(), os.path.join(RESULT_ROOT, "rnn", "rnn_car_detector.pth"))
    pd.DataFrame(train_log).to_csv(os.path.join(RESULT_ROOT, "rnn", "train_log.csv"), index=False)
    # 测试并保存结果
    test_rnn(model, device)


def test_rnn(model, device):
    ...
    with torch.no_grad():
        for imgs, labels, img_names in test_loader:
            ...
            cv2.putText(img, f"{cls_name} {conf:.2f}", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
            cv2.imwrite(os.path.join(RESULT_ROOT, "rnn", f"det_{img_names[0]}"), img)
    ...
    pd.DataFrame(result_list).to_csv(os.path.join(RESULT_ROOT, "rnn", "detection_result.csv"), index=False)
    print("RNN检测结果已保存至:", os.path.join(RESULT_ROOT, "rnn"))